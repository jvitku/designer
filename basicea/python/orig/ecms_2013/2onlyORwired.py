...
def buildNeurons(net,N,matrix):
    ...
    for d in range(N):
        ...
        model.connect(inputs.getOrigin('out'),'FloatToSpike_%d'%d,transform=[[0, 0]]) # 2D IN to spikes
        ...
        result = getattr(outputt, 'swt_in_%d'%d)(0)     
        
        # wire OR module
        model.connect(orModule.getOrigin('output'),'FloatToSpike_%d'%d,transform=[[0]])  # OR to neuron
        model.connect('SpikeToFloatInt_%d'%d,orModule.getTermination('inputs_%d0'%(d+2))) # neuron to OR1
        result = getattr(orModule, 'swt_inputs_%d0'%(d+2))(0)
        model.connect('SpikeToFloatInt_%d'%d,orModule.getTermination('inputs_%d1'%(d+2))) # neuron to OR2
        result = getattr(orModule, 'swt_inputs_%d1'%(d+2))(0)

...
class EventGenerator(simplemodule.SimpleModule):
    ...
    def termination_params(self,values):
        if(not self.started):
            ...
            self.threshold = self.il*wa[1]#+self.ir*wa[5]+wa[9]*values[0]; # simplification is here!!
...
while ea.wantsEval():
    ...
    if (ea.actualOne() == (popsize-1)):
        ...
        sx.save(ea.generation(),fit,ea.getIndNo(ea.getBest()).m.getVector())
...
print 'best fitness is: ' +repr(ind.getFitness().get());

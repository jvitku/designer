# evolution of hybrid neural architecture
# script is made for home computer, to run 5 experiments in a row (2simulations in parallel)
...
def buildNeurons(net,N,matrix):
    ...
    for d in range(N):
        ...
        model.connect(inputs.getOrigin('out'),'FloatToSpike_%d'%d,transform=[inToD])        # 2D IN to spikes
        ...
        if d==0:
            ...
            nto2 = hiddenW[6][1];
        # wire OR module
        model.connect(orModule.getOrigin('output'),'FloatToSpike_%d'%d,transform=[[otn]])     # OR to neuron
...
def buildExperiment(ind):
    ...
    generator=FunctionInput('generator',[FourierFunction(.5, 7,.7, 12),
        FourierFunction(.5, 7, .7, 112)],Units.UNK)
    net.add(generator);
    # model
...
for expNo in range(numRuns):
    ...
    expNooo = expNo*10000000+round(1000000*random.random(),0);   # generate some pseuso-number for text file data
    print expNo
